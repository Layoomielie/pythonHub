...
class ZhihuSpider(Spider):
    ...
    def parse_follows(self, response):
        ...
        if 'paging' in results.keys() and results.get('paging').get('is_end') == False:
            ...
            Num = Num + 20;
            yield Request(
                self.follows_url.format(user=self.start_user, include=self.follows_query, offset=Num, limit=20),
                self.parse_follows, meta={'Num': Num})
    ...
    def parse_followers(self, response):
        ...
        if 'paging' in results.keys() and results.get('paging').get('is_end') == False:
            ...
            Num = Num + 20;
            yield Request(
                self.followers_url.format(user=self.start_user, include=self.followers_query, offset=Num, limit=20),
                self.parse_followers, meta={'Num': Num})
